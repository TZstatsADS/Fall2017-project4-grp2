...
neighbor_select <- function(mat, neighbor){
  ...
  for (i in 1:rownum){
    ...
    # for spearman
    neighbor_row_spearman <- filter(mat,i==user)%>%arrange(desc(spear_corr))%>%select(i,j)%>%head(neighbor)

    mat_spearman[marker:(marker+neighbor-1),1:2] <- as.matrix(neighbor_row_spearman)
    
    
    # for vector similarity
    neighbor_row_vector <- filter(mat,i==user)%>%arrange(desc(vec_sim))%>%select(i,j)%>%head(neighbor)

     mat_vector[marker:(marker+neighbor-1),1:2] <- as.matrix(neighbor_row_vector)   
